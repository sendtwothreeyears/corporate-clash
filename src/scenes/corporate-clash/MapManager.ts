  ...
  render(world: CorporateWorld, renderer: Renderer): void {
    ...
    for (let row = 0; row < world.grid.length; row++) {
      for (let col = 0; col < world.grid[row].length; col++) {
        ...
        renderer.drawRect(LEFT_PANEL_WIDTH + col * CELL_SIZE, row * CELL_SIZE, CELL_SIZE, CELL_SIZE, color);
      }
    }

    if (world.hoveredTile) {
      const { row, col } = world.hoveredTile;
      renderer.drawRect(LEFT_PANEL_WIDTH + col * CELL_SIZE, row * CELL_SIZE, CELL_SIZE, CELL_SIZE, 0xffffff); // light overlay
    }
  }
}
