...
export class RenderManager implements Manager {
  render(world: SnakeWorld, renderer: Renderer): void {
    ...
    for (let i = 1; i < world.player.segments.length; i++) {
      renderer.drawRect(world.player.segments[i].x, world.player.segments[i].y, 1, 1, 0x44cc44);
    }

    // Snake head
    if (world.player.segments.length > 0) {
      renderer.drawRect(world.player.segments[0].x, world.player.segments[0].y, 1, 1, 0x22aa22);
    }
  }
}
